...
function _httpJSON(method, url, data, callback) {
    ...
    $.ajax(opt).done(function (r) {
        ...
        return callback({'error': 'http_bad_response', 'data': '' + jqXHR.status, 'message': ''+jqXHR.statusText});
    });
}
...
function search(){
		...
		$.ajax({
			url: context+"/service/solr/all/select",
			type:"post",
			data : {//start和rows参数表示本次搜索从第start条开始，搜索rows条，分页时使用
				"start" : pageNum*10,
				"rows" : 10,
				q : q,
				wt : 'json',
				indent : true,
				fq : 'groupname:' + groupname + ' AND ' + 'tablename:'
						+ tablename,
				'hl.fl':'*',
				hl:true,
				'json.wrf' : 'success'
